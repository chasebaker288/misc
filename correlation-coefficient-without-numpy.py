...
def correlation(data_set_1, data_set_2):
	if len(data_set_1) != len(data_set_2):
		return None  # I'm not sure if my reasoning behind this is all that solid. My thinking is that if the two data sets don't pair off, then there can't be a correlation.
	else:
		uppr, lowx, lowy = [], [], []
